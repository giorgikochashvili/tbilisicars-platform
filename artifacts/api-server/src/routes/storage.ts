 ...
 *
 * Auth note: requireAdmin is intentionally NOT used here.
 * All object keys are opaque UUIDs generated by getObjectEntityUploadURL() —
 * they are unguessable and not enumerable. Browser <img> tags (used for booking
 * photo thumbnails in the CRM) do not reliably send session cookies on iOS Safari
 * (especially in private browsing), so auth-gating this route causes 401 → blank
 * photos for staff. Security relies on UUID opacity rather than session auth.
 *
 * IMPORTANT: Only use this route for non-sensitive opaque assets (photos etc.).
 * If you need to serve sensitive files by a predictable path, add a separate
 * protected route rather than reusing this one.
 */
