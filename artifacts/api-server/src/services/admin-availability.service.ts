...
// ─── Add vehicle model to group (with 409 conflict detection) ─────────────────

export async function addModelToGroup(
  groupId: number,
  vehicleModelId: number,
  actorId: number | null,
): Promise<{ success: boolean; conflictGroupId?: number; conflictGroupName?: string }> {
  ...
  const [existing] = await db
    ...
    .leftJoin(
      availabilityGroupTable,
      eq(availabilityGroupTable.id, availabilityGroupVehicleModelTable.groupId),
    )
    .where(
      eq(availabilityGroupVehicleModelTable.vehicleModelId, vehicleModelId),
    );
  ...
  if (existing) {
    return {
      success: false,
      conflictGroupId: existing.groupId,
      conflictGroupName: existing.groupName ?? undefined,
    };
  }

  await db.insert(availabilityGroupVehicleModelTable).values({
    groupId,
    vehicleModelId,
  });
  ...
  logAudit({
    ...
    entityId: vehicleModelId,
    action: "CREATE",
    summary: `Added vehicleModelId=${vehicleModelId} to groupId=${groupId}`,
    afterData: { groupId, vehicleModelId },
  });
