...
interface BookingRow {
  ...
  customer: { id: number; fullName: string | null; email: string | null };
...
function StatCard({ title, value, icon: Icon, testId, isLoading }: {
  title: string; value: string | number | undefined; icon: React.ComponentType<{ className?: string }>; testId: string; isLoading?: boolean;
}) {
  return (
...
function ActivityTable({ title, bookings, isLoading, emptyMessage, timeKey }: {
  title: string; bookings?: BookingRow[]; isLoading: boolean; emptyMessage: string; timeKey: "pickup" | "dropoff";
}) {
  return (
        ...
        <Table>
          <TableHeader className="bg-background/80 sticky top-0 backdrop-blur-xl z-10">
            <TableRow className="border-border/40 hover:bg-transparent">
              <TableHead className="w-[70px] text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ref</TableHead>
              ...
              <TableHead className="w-[70px] text-xs font-semibold uppercase tracking-wider text-muted-foreground">Time</TableHead>
              <TableHead className="text-right w-[100px] text-xs font-semibold uppercase tracking-wider text-muted-foreground">Status</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading
                    ...
                    <TableRow key={b.id} className="border-border/20 hover:bg-muted/30 transition-colors cursor-default">
                      ...
                      <TableCell>
                        {b.vehicle ? (
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-mono text-muted-foreground px-1.5 py-0.5 bg-background border border-border/50 rounded inline-flex w-fit mt-0.5">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                          <span className="truncate max-w-[100px] text-foreground/80">{b.pickupLocation.name}</span>
                          <ArrowRightLeft className="w-3 h-3 flex-shrink-0 text-primary/50" />
                          <span className="truncate max-w-[100px] text-foreground/80">{b.dropoffLocation.name}</span>
                        </div>
                      </TableCell>
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          <StatCard title="Total" value={summaryQuery.data?.total} icon={CalendarClock} testId="stat-total" isLoading={summaryQuery.isLoading} />
          <StatCard title="Revenue" value={formatMoney(summaryQuery.data?.totalRevenue)} icon={CreditCard} testId="stat-revenue" isLoading={summaryQuery.isLoading} />
          <StatCard title="Pending" value={summaryQuery.data?.pending} icon={PlayCircle} testId="stat-pending" isLoading={summaryQuery.isLoading} />
          <StatCard title="Confirmed" value={summaryQuery.data?.confirmed} icon={CheckCircle2} testId="stat-confirmed" isLoading={summaryQuery.isLoading} />
          <StatCard title="Delivered" value={summaryQuery.data?.delivered} icon={Flag} testId="stat-delivered" isLoading={summaryQuery.isLoading} />
          <StatCard title="Returned" value={summaryQuery.data?.returned} icon={RotateCcw} testId="stat-returned" isLoading={summaryQuery.isLoading} />
          <StatCard title="Canceled" value={summaryQuery.data?.canceled} icon={XCircle} testId="stat-canceled" isLoading={summaryQuery.isLoading} />
          <StatCard title="No Show" value={summaryQuery.data?.noShow} icon={UserX} testId="stat-noshow" isLoading={summaryQuery.isLoading} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 h-[420px]">
          <ActivityTable
